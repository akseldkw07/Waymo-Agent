    ...
    for ite in range(iterations):
        ...
        for _ in range(numtrajs):
            ...
            while not done:
                ...
                actions = [np.random.choice(env.action_space.n, p=probs[i]/np.sum(probs[i])) for i in range(num_taxis)]
                for i in range(num_taxis):
...
def main():
    ...
    clip_curve = train_agent(CityTaxiEnv(num_taxis=num_taxis), clip_actor, clip_baseline,
                             iterations, numtrajs, gamma, label="PPO-Clip")
    ...
    vanilla_curve = train_agent(CityTaxiEnv(num_taxis=num_taxis), vanilla_actor, vanilla_baseline,
                                iterations, numtrajs, gamma, label="Vanilla PG")
    ...
    plot_policy_comparison(
        {"PPO-Clip": clip_curve, "Vanilla PG": vanilla_curve},
        {"PPO-Clip": clip_score, "Vanilla PG": vanilla_score}
    )
